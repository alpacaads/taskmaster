...
2. **Fight — you need these supplies** `COMBAT`
   - ⚔ combat _(default, mission partner = maya, mission partner = ren, solo mission, saved Nora, bring Nora on mission, told Vega, chore: medbay, chore: perimeter, chore: kitchen, exposed traitor, killed traitor, romance Ren)_: enemy `bandit_younger` · risky → win `after_ambush_fight` / lose `death`
   - ⚔ combat _(with Maya companion, romance Maya)_: enemy `bandit_younger` · risky → win `after_ambush_fight` / lose `death`
   - ⚔ combat _(rested in car)_: enemy `bandit_younger` → win `after_ambush_fight` / lose `death`
...
1. **Fight for your life** `COMBAT`
   ...
   - ⚔ combat _(default, mission partner = maya, mission partner = ren, solo mission, saved Nora, bring Nora on mission, rested in car, told Vega, chore: medbay, chore: perimeter, chore: kitchen, exposed traitor, killed traitor, romance Ren)_: enemy `bandit_younger` → win `sacrifice_loot` / lose `death`
   - ⚔ combat _(with Maya companion, romance Maya)_: enemy `bandit_younger` → win `sacrifice_loot` / lose `death`
...
1. **Hold the wall.** `COMBAT`
   ...
   - ⚔ combat _(default, with Maya companion, mission partner = maya, mission partner = ren, solo mission, saved Nora, bring Nora on mission, rested in car, told Vega, chore: medbay, chore: perimeter, chore: kitchen, killed traitor, romance Maya, romance Ren)_: enemy `walker_horde` → win `post_horde_win` / lose `post_horde_lose`
   - ⚔ combat _(exposed traitor)_: enemy `walker_horde` → win `post_horde_win` / lose `post_horde_lose`
